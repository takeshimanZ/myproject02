...
farmer_x = random.randint(0, screen_width - farmer_width)
farmer_y = -farmer_height - 60  # より上に設定
farmer_speed = 5
farmer_spawn_timer = pygame.time.get_ticks()
farmer_active_time = 10000  # 農夫が画面上に表示される時間（ミリ秒）

# ライフルの設定
rifle_radius = 15  # 弾の大きさ
...
sheep_frequency = 25
sheep_list = []
sheep_sounds = [pygame.mixer.Sound("sheep1.mp3"), pygame.mixer.Sound("sheep2.mp3"), pygame.mixer.Sound("sheep3.mp3")]
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if title_screen:
                ...
                wolf_y = screen_height - wolf_height - 10
                farmer_x = random.randint(0, screen_width - farmer_width)
                farmer_y = -farmer_height - 50  # より上に設定
                farmer_spawn_timer = pygame.time.get_ticks()
                farmer_active_timer = 0  # 農夫の活動時間のタイマーを初期化
                farmer_speed_x = 3  # 初期化
                farmer_move_direction = 1  # 初期化
                farmer_shoot_frequency = 50  # 初期化
                farmer_shoot_timer = 0  # 初期化
            elif game_over:
    ...
    if not title_screen:
        ...
        if farmer_active_timer and pygame.time.get_ticks() - farmer_active_timer < farmer_active_time:
            ...
            if farmer_shoot_timer >= farmer_shoot_frequency:
                # 弾を発射
                farmer_shoot()
                if not game_over:  # ゲームオーバーでない場合のみ発言
                    # 農夫の発言
                    farmer_speak()
