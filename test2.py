...
title_background_image = pygame.image.load("title_background.jpg")
background_image = pygame.image.load("background.jpg")
wolf_image = pygame.image.load("wolf.png")
sheep_image = pygame.image.load("sheep.png")
farmer_image = pygame.image.load("farmer.png")  # 新しく追加した農夫の画像
...
# 効果音の読み込み
wolf_sound = pygame.mixer.Sound("wolf_bite.mp3")
...
while True:
    ...
    if not title_screen and not game_over:
        ...
        if score % 100 == 0 and score != 0:
            # scoreが100ごとに農夫がしゃべり、登場
            if pygame.time.get_ticks() - farmer_speak_timer > 2000:
                farmer_speak_timer = pygame.time.get_ticks()
                random.choice(farmer_speak_sounds).play()

            if pygame.time.get_ticks() - farmer_spawn_timer > 2000:
                farmer_spawn_timer = pygame.time.get_ticks()
                farmer_x = screen_width  # 右端から登場

        farmer_x -= farmer_speed
        if farmer_x < -farmer_width:
            farmer_x = -farmer_width
            farmer_spawn_timer = pygame.time.get_ticks()

        if remaining_time <= 0:
            game_over = True

    screen.fill(white)  # 白色の背景を表示
