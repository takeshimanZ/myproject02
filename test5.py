...
resume_font = pygame.font.SysFont(None, 60)
pause_text = pause_font.render("Pause", True, black)
resume_text = resume_font.render("Press 'R' to resume", True, black)
title_back_text = resume_font.render("Press 'T' for Title", True, black)
restart_text = resume_font.render("Press 'G' to restart", True, black)
setting_text = resume_font.render("Press 'S' for Setting", True, black)
...
title_back_rect = title_back_text.get_rect(center=(screen_width // 2, screen_height // 1.8))
restart_rect = restart_text.get_rect(center=(screen_width // 2, screen_height // 1.5))
setting_rect = setting_text.get_rect(center=(screen_width // 2, screen_height // 1.3))

# 設定画面の設定
setting_screen = False
setting_bg_color = (200, 200, 200)
slider_color = (0, 128, 255)
bgm_slider_width = 300
sfx_slider_width = 300
slider_height = 20
bgm_slider_x = (screen_width - bgm_slider_width) // 2
sfx_slider_x = (screen_width - sfx_slider_width) // 2
slider_y = screen_height // 3
bgm_slider_rect = pygame.Rect(bgm_slider_x, slider_y, bgm_slider_width, slider_height)
sfx_slider_rect = pygame.Rect(sfx_slider_x, slider_y + 100, sfx_slider_width, slider_height)
bgm_volume = 0.5
sfx_volume = 0.5
...
while True:
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if title_screen or game_over:
            ...
            elif paused and event.key == pygame.K_t:
                paused = False
                pygame.mixer.music.stop()
                pygame.mixer.music.load(bgm_folder + "title_bgm.mp3")
                pygame.mixer.music.play(-1)
                title_screen = True
            elif paused and event.key == pygame.K_g:
                paused = False
                pygame.mixer.music.stop()
                pygame.mixer.music.load(bgm_folder + "ingame_bgm.mp3")
                pygame.mixer.music.play(-1)
                ...
                wolf_x = screen_width // 2 - wolf_width // 2
                wolf_y = screen_height - wolf_height - 10
            elif paused and event.key == pygame.K_s:
                setting_screen = not setting_screen

    keys = pygame.key.get_pressed()
    if not title_screen and not game_over and not paused:
    ...
    if title_screen:
        ...
        screen.fill(setting_bg_color)
        screen.blit(pause_text, pause_rect)
        screen.blit(resume_text, resume_rect)
        screen.blit(title_back_text, title_back_rect)
        screen.blit(restart_text, restart_rect)
        screen.blit(setting_text, setting_rect)
    if setting_screen:
        pygame.draw.rect(screen, slider_color, bgm_slider_rect)
        pygame.draw.rect(screen, slider_color, sfx_slider_rect)
        pygame.draw.rect(screen, black, bgm_slider_rect, 2)
        pygame.draw.rect(screen, black, sfx_slider_rect, 2)
        pygame.draw.rect(screen, black, bgm_slider_rect)
        pygame.draw.rect(screen, black, sfx_slider_rect)
        pygame.draw.rect(screen, slider_color, (bgm_slider_x, slider_y, bgm_volume * bgm_slider_width, slider_height))
        pygame.draw.rect(screen, slider_color, (sfx_slider_x, slider_y + 100, sfx_volume * sfx_slider_width, slider_height))
        draw_text("BGM Volume", pygame.font.SysFont(None, 30), black, bgm_slider_x, slider_y - 50)
        draw_text("SFX Volume", pygame.font.SysFont(None, 30), black, sfx_slider_x, slider_y + 50)
    else:
